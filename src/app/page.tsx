...
const JOURNEY = [
  ...
  },
  {
    ...
    icon: ListChecks,
    gradient: "bg-orange-100 text-orange-700",
  },
  {
    when: "Every week",
    title: "Review & adapt",
  ...
  },
  {
    ...
    icon: Rocket,
    gradient: "bg-cyan-100 text-cyan-700",
  },
];
...
const MODULES = [
  { title: "AI Onboarding & Assessment", text: "Your current course, target career, interests and daily hours produce your initial readiness score and gap analysis.", icon: Sparkles, gradient: "bg-violet-100 text-violet-700" },
  { title: "Personalized Roadmap", text: "A role-specific 12-week skill tree with prerequisites that adapts to your performance.", icon: Map, gradient: "bg-blue-100 text-blue-700" },
  { title: "Daily AI To-Do System", text: "Learning, coding, project, profile and opportunity tasks with automatic rollover.", icon: CalendarCheck, gradient: "bg-emerald-100 text-emerald-700" },
  { title: "Resume Builder & AI Review", text: "ATS-oriented analysis, missing-skill detection and role-specific suggestions.", icon: FileText, gradient: "bg-orange-100 text-orange-700" },
  { title: "LinkedIn Review", text: "Headline, About, skills and completeness with positioning recommendations.", icon: Globe, gradient: "bg-blue-100 text-blue-700" },
  { title: "GitHub Review", text: "Repository quality, README and activity analysis with a portfolio readiness score.", icon: GitBranch, gradient: "bg-slate-100 text-slate-600" },
  { title: "Project Engine", text: "Skill-gap-driven project picks with milestones that become resume evidence.", icon: FolderGit2, gradient: "bg-rose-100 text-rose-600" },
  { title: "Coding Practice", text: "Daily problems on your weak topics with consistency and improvement analytics.", icon: Code2, gradient: "bg-cyan-100 text-cyan-700" },
  { title: "Placement Preparation", text: "Aptitude, DSA, technical subjects, MCQs, HR prep and company-specific plans.", icon: Target, gradient: "bg-orange-100 text-orange-700" },
  { title: "Hackathons & Events", text: "Competition discovery, participation tracking and an achievements portfolio.", icon: Trophy, gradient: "bg-amber-100 text-amber-700" },
  { title: "Opportunity Gateway", text: "Smart redirection to Internshala, LinkedIn, Naukri, Indeed and Unstop — with context.", icon: ExternalLink, gradient: "bg-indigo-100 text-indigo-700" },
  { title: "Free Interview System", text: "AI mock technical and HR interviews with scores, weak areas and next practice.", icon: Mic2, gradient: "bg-rose-100 text-rose-600" },
];
...
export default async function LandingPage() {
  ...
  return (
    <div className="flex-1 bg-surface text-slate-900 overflow-hidden">
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(JSON_LD) }}
      />
      {/* ── Header ── */}
      <header className="sticky top-0 z-30 border-b border-slate-100/80 bg-surface/80 backdrop-blur-xl animate-fade-in-down">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <Link href="/" className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-sm font-bold text-white shadow-sm shadow-indigo-900/10">
              CO
            </div>
            <span className="text-lg font-serif font-semibold text-slate-900">Career OS</span>
          </Link>
      ...
      <section className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 pt-20 pb-16 lg:grid-cols-2">
        <div className="animate-fade-in-up">
          ...
          <h1 className="font-serif text-4xl font-medium tracking-tight sm:text-5xl lg:text-6xl leading-[1.15]">
            ...
            <br />
            <span className="text-gradient-brand text-gradient-animated animate-fade-in-up delay-200">
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-6 py-10 text-center sm:grid-cols-3 lg:grid-cols-5">
          {STATS.map((s, i) => (
            <div key={s.label} className="animate-fade-in-up group" style={{ animationDelay: `${i * 100}ms` }}>
              <p className="font-serif text-3xl font-semibold text-slate-900 transition-transform group-hover:scale-110 sm:text-4xl">
                {s.value}
              </p>
      ...
      <section id="journey" className="mx-auto max-w-6xl px-6 py-20 scroll-mt-20">
        <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
          ...
          <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">From Day 1 to placement, one continuous loop</h2>
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {JOURNEY.map((step, i) => (
            <Reveal key={step.title} delay={i * 80} className="h-full">
              <div className="group relative flex h-full flex-col rounded-2xl border border-slate-100 bg-surface p-6 shadow-sm card-interactive">
                <div className="flex items-center gap-3">
                  <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${step.gradient} transition-transform group-hover:scale-110 group-hover:rotate-3`}>
      ...
      <section className="relative border-y border-slate-100/80 bg-gradient-to-b from-slate-50/80 to-white py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
            ...
            <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">Not another course site. An engine that decides.</h2>
            <p className="mt-4 text-lg text-slate-600">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-4">
            {PHASES.map((p, i) => (
              <Reveal key={p.name} delay={i * 100} className="h-full">
                <div className="group flex h-full flex-col rounded-2xl border border-slate-100 bg-surface p-6 shadow-sm card-interactive">
                  ...
                  <h3 className="mt-3 font-serif text-lg font-medium text-slate-900">{p.name}</h3>
      ...
      <section id="modules" className="mx-auto max-w-6xl px-6 py-20 scroll-mt-20">
        <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
          ...
          <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">Everything a student needs, orchestrated in one place</h2>
          <p className="mt-4 text-lg text-slate-600">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {MODULES.map((m, i) => (
            <Reveal key={m.title} delay={(i % 3) * 80} className="h-full">
              <div className="group relative flex h-full flex-col rounded-2xl border border-slate-100 bg-surface p-5 shadow-sm card-interactive overflow-hidden">
                <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${m.gradient} transition-all duration-300 group-hover:scale-110`}>
      ...
      <section id="score" className="relative border-y border-slate-100/80 bg-gradient-to-b from-slate-50/80 to-white py-20 scroll-mt-20">
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-6 lg:grid-cols-2">
          <div className="animate-fade-in-left">
            ...
            <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">A score you can argue with — because it&apos;s evidence-based</h2>
            ...
            <div className="mt-6 space-y-2">
              {READINESS_DIMENSIONS.map((d, i) => (
                <Reveal key={d.key} delay={i * 50}>
                  <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-surface px-4 py-2.5 shadow-sm transition-all hover:border-indigo-200 hover:shadow-md hover:translate-x-0.5">
          ...
          <div className="animate-slide-in-right">
            <div className="rounded-2xl border border-slate-200 bg-surface p-6 shadow-sm shadow-indigo-900/10">
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-sm font-medium text-slate-500">Placement readiness</p>
                  <p className="mt-1 font-serif text-5xl font-semibold text-slate-900">
              ...
              <div className="mt-6 space-y-4">
                {[
                  { label: "Core technical skills", pct: 42, color: "bg-indigo-500" },
                  { label: "Coding / DSA", pct: 42, color: "bg-cyan-600" },
                  { label: "Projects / portfolio", pct: 42, color: "bg-emerald-600" },
                ].map((row) => (
                  <div key={row.label}>
                    ...
                    <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
                      <div
                        className={`h-full rounded-full ${row.color} animate-progress-fill`}
      ...
      <section id="gateway" className="mx-auto max-w-6xl px-6 py-20 scroll-mt-20">
        <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
          ...
          <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">We don&apos;t rebuild the web. We route it.</h2>
          <p className="mt-4 text-lg text-slate-600">
          ...
          {GATEWAYS.map((g, i) => (
            <Reveal key={g.name} delay={(i % 3) * 80} className="h-full">
              <a
                ...
                className="group flex h-full items-center gap-4 rounded-2xl border border-slate-100 bg-surface p-5 shadow-sm transition-all duration-300 hover:border-slate-200 hover:shadow-lg hover:-translate-y-0.5"
              >
                <div
                  className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-base font-bold text-white shadow-sm transition-transform group-hover:scale-110 group-hover:rotate-3"
      ...
      <section id="colleges" className="relative border-y border-slate-100/80 bg-gradient-to-b from-slate-50/80 to-white py-20 scroll-mt-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
            ...
            <h2 className="mt-3 font-serif text-3xl font-medium tracking-tight sm:text-4xl">Institution intelligence without surveillance</h2>
            <p className="mt-4 text-lg text-slate-600">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-4">
            {[
              ...
              <Reveal key={s.label} delay={i * 80} className="h-full">
                <div className="group flex h-full flex-col justify-center rounded-2xl border border-slate-100 bg-surface p-5 text-center shadow-sm card-interactive">
                  <p className="font-serif text-3xl font-semibold text-slate-900 transition-transform group-hover:scale-110">
                    {s.value}
                  </p>
                  <p className="mt-1 text-xs font-medium text-slate-500">{s.label}</p>
                </div>
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-slate-100 bg-surface shadow-sm animate-fade-in-up delay-400">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100">
                            <div className="h-full rounded-full bg-indigo-500" style={{ width: `${row.score}%` }} />
                          </div>
      ...
      <section id="testimonials" className="mx-auto max-w-6xl px-6 py-20 scroll-mt-20">
        <div className="mx-auto max-w-2xl text-center animate-fade-in-up">
          ...
          <h2 className="font-serif text-3xl font-medium tracking-tight sm:text-4xl">Built for consistency, not intensity</h2>
          <p className="mt-4 text-lg text-slate-600">
            ...
            <Reveal key={t.author} delay={i * 100} className="h-full">
              <div className="group relative flex h-full flex-col rounded-2xl border border-slate-100 bg-surface p-6 shadow-sm card-interactive">
                ...
                <div className="mt-5 border-t border-slate-100 pt-4">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full ${t.color} text-sm font-bold text-white shadow-sm shadow-indigo-900/10 transition-transform group-hover:scale-110`}>
                      {t.author}
                    </div>
                    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-surface/10 blur-2xl animate-float" />
        <div className="absolute -bottom-20 -left-20 h-64 w-64 rounded-full bg-surface/10 blur-2xl animate-float-delayed" />
        <div className="relative mx-auto max-w-3xl px-6 py-20 text-center">
          <div className="animate-fade-in-up">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-surface/20 px-3 py-1 text-xs font-semibold text-white/90 mb-6 backdrop-blur-sm">
              <Star className="h-3 w-3" /> Free for students
            </div>
            <h2 className="font-serif text-3xl font-medium text-white sm:text-4xl">Free for students. Always.</h2>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4 animate-fade-in-up delay-200">
            <Link href="/register">
              <Button size="xl" className="bg-surface text-indigo-700 hover:bg-indigo-50 shadow-xl shadow-black/10 group">
                Build my career OS
                <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Button>
            </Link>
            <Link href="/login">
              <Button size="lg" variant="outline" className="border-white/30 text-white hover:bg-surface/10 backdrop-blur-sm">
                Sign in
              </Button>
            </Link>
          </div>
...
function DashboardPreview() {
  ...
  return (
    <div className="relative rounded-2xl border border-slate-200 bg-surface p-6 shadow-sm shadow-indigo-900/10 animate-float">
      ...
      <div className="mt-4 flex items-end justify-between">
        <div className="flex items-end gap-1">
          <span className="font-serif text-5xl font-semibold text-slate-900">58</span>
      ...
      <div className="mt-4">
        ...
        <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
          <div className="h-full w-[68%] rounded-full bg-indigo-500 animate-progress-fill" />
        </div>
      </div>
    </div>
  );
